...
if [ -f "$instdir/foglight_startup.jar" ]; then
...
if [ -f "${instdir}/foglight_startup.jar" ]; then
"${JRE}/bin/java" -server $PLATFORM_JAVA_ARGS -jar "${instdir}/../client/foglight-cmdline-client.jar" "$@"
else
"${JRE}/bin/java" -server $PLATFORM_JAVA_ARGS -jar "${instdir}/foglight-cmdline-client.jar" "$@"
fi
